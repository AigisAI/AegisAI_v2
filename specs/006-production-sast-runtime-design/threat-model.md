...
| T055 rollback sequencing or append-only ledger forgery | A candidate continues after fencing, in-flight work is hidden, a caller chooses a rollback target, the baseline activates out of order, or an audit entry rewrites/forks history | Exact candidate/baseline release-set binding; ordered five-phase prefix per profile; in-flight before/after and abort proof; derived last-known-good target; zero post-fence invocation; baseline `STANDBY -> ACTIVE` only through a digest-chained ledger entry bound to the audit ref | Missing/reordered/duplicated phase, release-set drift, post-fence invocation, incomplete abort/cleanup, caller-target substitution, previous-digest/content/audit drift, and forged aggregate tests; zero T056 authority on failure |
...
24. T055 supply-chain and rollback evidence cannot inherit authority from an aggregate or local
    ...
    rollback with queue fencing, in-flight abort, derived last-known-good state, and an append-only
    ledger. A complete external pass advances only to T056.
