...
| Canonical scan-key version collision | Existing v1 work is replayed after tenant-policy receipt identity is added to the key preimage | Explicit `sast-canonical-scan-key-v2`; migration blocks while any prior SAST plan or reservation is non-terminal; terminal v1 history stays immutable | Dirty-cutover migration rejection and clean drained/canceled cutover checks |
