...
| Isolated qualification clock or retroactive-approval replay | A caller replays expired evidence with a historical evaluation time or signs the plan after provider execution begins | Verifier-owned trusted UTC evaluation time with no caller override; dependency/receipt freshness checked at that instant; both detached approvals must strictly predate the earliest valid receipt start | CLI time-override rejection, trusted-now bounds, stale replay, and equal/post-start approval rejection tests; zero T054 authority on failure |
| End-to-end qualification aggregate forgery or retry erasure | A caller submits favorable precision/recall/p95 totals, omits a failed attempt or slow bucket, substitutes baseline/candidate assets or hardware, reuses a sandbox/attestation, duplicates a cell, or presents a partial set as T054 success | Exact 3,462-cell manifest; distinct digest-bound arms; fixed nine buckets and 30 runs per arm; full one/two-attempt chains; globally unique identities; provider/runtime/telemetry signatures; receipt-only denominator and nearest-rank percentile recomputation; exact profile resource and zero-tolerance gates | Full-pass, partial-pending, >2% retained-retry, duplicate/reuse, candidate/baseline/hardware drift, sample omission, latency/resource threshold, zero-event, signature tamper, package-byte/path/link/encoding, and authority-widening tests; no aggregate input field exists |
| T054 prerequisite or retroactive-approval replay | A repository-only or partial T053 result starts T054, an entry attestation binds another result/manifest, approvals are added after execution, or stale receipts are verified with a caller clock | T053 `PASSED` plus `t054EntryAuthorized` and Qualification Authority Ed25519 attestation required before plan construction; exact T053/T054/dependency binding; verifier-owned UTC time; both approvals strictly before the earliest attempt | Blocked-without-T053, pending/failed prerequisite, entry signature/result/manifest drift, CLI time override, expired window, future receipt, and equal/post-start approval tests; zero T055 or deployment authority on failure |
| T054 self-selected trust root or unsigned artifact substitution | A caller supplies both a malicious trust bundle and its dependency digest, uses plausible refs without their envelopes, or inserts an invalid artifact/provenance signature behind a valid aggregate | Trust-bundle bytes must match independently configured `SAST_T054_TRUST_POLICY_DIGEST`; load each signature envelope and provenance statement; recompute both envelope digests; bind provenance subject/source/builder/materials; verify both per-artifact `SUPPLY_CHAIN_AUTHORITY` Ed25519 signatures; then verify the closed set signature | Missing payload, attacker-selected trust root, invalid artifact/provenance/set signature, missing/duplicate/mismatched artifact, subject/source/materials drift, and envelope-digest tamper tests; plan creation denied |
| T054 provider qualification transfer | A valid T053 pass for provider A is replayed with a T054 dependency set and receipts from provider B | Load and validate the exact T053 dependency set named by the passing result; bind its digest, provider ID, and provider-adapter ref into the T054 plan and require exact equality with T054 | Cross-provider and cross-adapter plan-construction rejection plus receipt provider binding; zero T055 authority |
| T055 supply-chain envelope substitution or allowlist bypass | A mounted T054 artifact changes after qualification, a signature/provenance boolean hides a substituted envelope, an unlisted component runs, or stale/external vulnerability data bypasses the internal mirror | Exact 169-cell denominator; reload and digest-recompute every artifact and envelope; rebind provenance; independent Ed25519 verification; read-only mount rehash; closed allowlist; stale and network-enrichment rejection before invocation/egress | Real-signature, mount/digest/signature/provenance substitution, unlisted component, DB freshness/network, schema drift, exact 115 pre-execution rejection and 39 invocation tests; zero customer execution, egress, or production mutation |
| T055 rollback sequencing or append-only ledger forgery | A candidate continues after fencing, in-flight work is hidden, a caller chooses a rollback target or arbitrary syntactically valid prior digest, the baseline activates out of order, or an audit entry rewrites/forks history | Exact candidate/baseline release-set binding; ordered five-phase prefix per profile; in-flight before/after and abort proof; derived last-known-good target; zero post-fence invocation; Qualification Authority plus Supply Chain Authority authenticate each durable prior head digest/sequence/ref before plan approval; baseline `STANDBY -> ACTIVE` must reference that exact plan-bound attestation and append exactly sequence + 1 | Missing/reordered/duplicated phase, release-set drift, post-fence invocation, incomplete abort/cleanup, caller-target or arbitrary-head substitution, head-signature/sequence/content/audit drift, and forged aggregate tests; zero T056 authority on failure |
| T055 trust-root or provider qualification transfer | An attacker supplies its own trust bundle, replays a T054 pass from another provider/adapter, signs after execution, reuses an identity, or treats local receipts as external evidence | Process-pinned `SAST_T055_TRUST_POLICY_DIGEST`; exact T054 manifest/result/dependency/verification/plan rebinding; same provider/adapter; Qualification Authority entry signature; pre-execution dual approval; three independent receipt signatures; verifier-owned clock and global single-use identities | Missing/untrusted T055 root, cross-provider/adapter and upstream-plan drift, entry/receipt signature tamper, retroactive approval, stale/future evidence, identity reuse, partial-pending, and repository-blocked tests; no deployment, Kubernetes, or readiness authority |
| T056 favorable aggregate or upstream measurement substitution | A caller submits passing recall/precision/latency/drill totals, rebinds a different T054/T055 result, or transfers a pass across provider/adapter boundaries | Exact T054/T055 manifest-entry-dependency-verification-plan-result chain; same provider/adapter; Qualification Authority entry signature; verifier recomputes all 34 upstream observations from signed measurements and accepts no aggregate verdict | Favorable-but-different observation, cross-provider/adapter, upstream digest drift, entry signature tamper, and exact 54-gate recomputation tests; mismatch is `NO_GO` |
| T056 missing, stale, unsigned, or not-applicable evidence laundering | A missing category or approval is called GO, stale evidence is replayed with a caller clock, a signature is replaced, or a mandatory gate is marked not applicable | Exactly six ordered category attestations; process-owned UTC time; digest-bound validity intervals; category-specific independent signatures; all 54 v1 gates mandatory; explicit pending versus no-go state split | Missing category/approval remains `PENDING_FINAL_EVIDENCE`; stale, future, malformed, unsigned, threshold-breaching, or `NOT_APPLICABLE` evidence is `NO_GO`; CLI time override is rejected |
| T056 trust-root or deployment-authority widening | Evidence chooses its own trust root, a GO record is mutated to claim deployment/Kubernetes/readiness authority, or repository fixtures are presented as production evidence | Independently pinned `SAST_T056_TRUST_POLICY_DIGEST`; immutable digest-bound plan and record; GO sets only 005 operations-entry authorization; every other authority bit fixed false; repository package is blocked without T055 | Attacker-root, Ed25519 tamper, record mutation, repository-blocked, and zero-authority tests; no Kubernetes execution or production readiness follows from T056 |
| 005 self-asserted or replayed T056 entry | A caller claims GO without evidence, replays an expired or different-contract record, changes provider/adapter/commit/rollback/approval bindings, or widens entry into deployment authority | No evidence-free preflight; fresh Qualification Authority Ed25519 entry attestation over exact T056 record/manifest/plan and current 005 contract; trusted UTC; immutable qualification binding; exact provider/adapter, approvals and rollback; bounded handoff; all live authority false | Real-signature passing path plus non-GO, invalid signature, expiry, provider/approval/rollback drift, extra-field and zero-authority rejection tests; no Kubernetes or provider call |
...
24. T055 supply-chain and rollback evidence cannot inherit authority from an aggregate or local
    fixture. It must reverify every T054 artifact against an independently pinned trust root, reject
    every non-allowlisted or network-dependent path before execution, and prove an exact ordered
    rollback with queue fencing, in-flight abort, derived last-known-good state, dual-signed prior
    ledger-head attestation, and an exact next append-only entry. A complete external pass advances
    only to T056.
25. T056 cannot inherit a verdict, trust root, clock, or deployment authority from submitted
    evidence. It must rebind the exact same-provider T054/T055 chain, verify independently pinned
    Ed25519 authorities, recompute all 54 mandatory gate outcomes, preserve missing categories or
    approvals as pending, fail malformed/stale/drifted/breached/`NOT_APPLICABLE` evidence as
    no-go, and issue at most a reference-only authorization to enter 005 deployment operations.
26. The 005 consumer cannot turn the T056 entry bit into a caller assertion. It must verify a fresh
    Qualification Authority Ed25519 attestation over the exact GO record, manifest, plan, commit,
    provider/adapter, rollback, kill-switch evidence, and current 005 contract; reconstruct one
    immutable binding under trusted UTC; reject expiry or drift; and keep every live-operation and
    readiness authority false through preflight and bounded handoff.
