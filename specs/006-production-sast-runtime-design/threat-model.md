...
| External lifecycle authority spoofing | Canary promotion, emergency suspension, or rollback occurs without the corresponding production decision | Exact per-edge authority type and digest-bound receipt; unavailable default authorities; T048-T050 install separate implementations | Missing/mismatched authority receipt tests and default-unavailable production path |
| Canonical scan-key version collision | Existing v2 work is replayed after lifecycle selection identity is added to the key preimage | Explicit `sast-canonical-scan-key-v3`; migration blocks while any prior SAST plan or reservation is non-terminal; terminal v2 history stays immutable | Dirty-cutover migration rejection and clean drained/canceled cutover checks |
