...
`AGENTS.md` and `specs/004-production-runtime-infrastructure/quickstart.md` as the execution
...
```text
AGENTS.md
specs/004-production-runtime-infrastructure/quickstart.md
...
- [ ] **Step 3: Point to canonical validation**

The validation section must point to:
...
```powershell
Select-String -Path 'README.md' -Pattern '004-production-runtime-infrastructure'
```
...
```powershell
Select-String -Path 'README.md' -Pattern 'agent-first MVP baseline|default implementation target.*001|default implementation target.*002|Oracle.*production topology'
```
...
claims live production Kubernetes cluster provisioning, provider-specific microVM rollout,
or production cluster credentials as local development defaults.
