...
function ruleBundle(
  ...
  return {
    ...
    manifestId: `sast-rule-bundle-manifest://${batchIndependentDigest(`${seed}-manifest`).slice('sha256:'.length)}`,
    manifestDigest: batchIndependentDigest(`${seed}-manifest`),
    verificationId: `sast-rule-bundle-verification://${batchIndependentDigest(`${seed}-manifest`).slice('sha256:'.length)}`,
    verificationDigest: batchIndependentDigest(`${seed}-verification`),
