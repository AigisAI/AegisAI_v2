...
class OpenGrepSarifCollector
  implements SastNormalizationJsonCollector<ParsedOpenGrepSarif>
{
  ...
  finish(): ParsedOpenGrepSarif | null {
    if (
    ...
    if (
      ...
      this.driverVersion !== this.scannerVersion
    ) {
      this.driverInvalid();
    }
    if (
